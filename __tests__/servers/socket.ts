...
import { api, config, chatRoom, Process } from "actionhero";
...
describe("Server: Socket", () => {
  ...
  test("actions will fail without proper params set to the connection", async () => {
  ...
  test("will limit how many simultaneous connections I can have", async () => {
    ...
    await new Promise((resolve) => {
      const checkResponses = (data) => {
        ...
        if (responses.length >= config.general.simultaneousActions + 1) {
  ...
  describe("maxDataLength", () => {
    test("will error If received data length is bigger then maxDataLength", async () => {
      let longMessage = "";
      for (let i = 0; i < 50; i++) {
        longMessage += uuid.v4();
      }
      ...
      expect(response.error).toEqual("DATA_LENGTH_TOO_LONG");
    });
  });

  // describe("custom data delimiter", () => {
  //   afterEach(() => {
  //     config.servers.socket.delimiter = "\n";
  //   });

  //   test("will parse custom newline data delimiter", async () => {
  //     config.servers.socket.delimiter = "xXxXxX";
  //     const response = await makeSocketRequest(
  //       client,
  ...
  describe("chat", () => {
    ...
    test("clients see an appropriate error when viewing rooms they are not in", async () => {
    ...
    describe("chat middleware", () => {
      ...
      beforeEach(async () => {
        await sleep(100); // wait sufficiently long for other room ops to complete
      });
