...
class SocketServer extends actionhero_1.Server {
    constructor() {
        super();
        this.type = "socket";
        this.attributes = {
    ...
    async start() {
        if (this.config.secure === false) {
            this.server = net.createServer(this.config.serverOptions, rawConnection => {
                this.handleConnection(rawConnection);
            });
        }
        else {
            this.server = tls.createServer(this.config.serverOptions, rawConnection => {
                this.handleConnection(rawConnection);
            });
        }
    ...
    async sendMessage(connection, message, messageId) {
        if (message.error) {
            message.error = actionhero_1.config.errors.serializers.servers
                ? await actionhero_1.config.errors.serializers.servers.socket(message.error)
